...
impl TextInput {
    pub fn new(placeholder: &str, cx: &mut Context<Self>) -> Self {
        Self {
            focus_handle: cx.focus_handle(), // Will be replaced in `new` or initialized properly if not using cx
            content: "".into(),
    ...
    // To support `cx.new(|cx| TextInput::new(...))` or similar patterns where we need cx
    // We typically implement a separate method or just initialize fields in the closure directly.
    // But here we can just use the struct update syntax in the caller or a helper.
    // The example code initializes `focus_handle` with `cx.focus_handle()`.
    ...
    pub fn set_text(&mut self, text: &str, cx: &mut Context<Self>) {
        ...
        self.selected_range = 0..0; // Reset cursor
        cx.notify();
    }
...
impl Render for TextInput {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .track_focus(&self.focus_handle.clone()) // Clone here
